...
public class CommentDAO {
   ...
   public Date getPostdate()
   {
	...
	public int write(String commentContent, String id, int num) {
		String sql="insert into comment1(commentID, commentContent, id, available, postdate, num) "
...
//			pstmt.setInt(1, getNewNext());
	...
	public ArrayList<Comment> getList(int commentID){	// 특정한 리스트를 받아서 반환
		String sql = "select * from (select * from comment1 where num < ? and available = 1 order by num desc) where rownum <= 10";	// 마지막 게시물 반환, 삭제가 되지 않은 글만 가져온다.
