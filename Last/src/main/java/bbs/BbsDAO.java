...
public class BbsDAO
{
   ...
   public int write(String title, String id, String content)
   {
	   ...
	   sql = "insert into board1(num, title, name, email, postdate, content, available) "
...
//         pstmt.setInt(1, getNext());
         ...
         pstmt.setInt(6, 1);   // 글의 유효번호 추후 delete가 아닌 유효성 상실로 삭제 사용
      ...
      ArrayList<Bbs> list = new ArrayList<Bbs>();
         try
         {
            ...
            while (rs.next())
            {
               ...
               bbs.setEmail(rs.getString(4));
               bbs.setPostdate(rs.getString(5));
    ...
    public ArrayList<Bbs> getSearch(String searchField, String searchText){	// 특정한 리스트를 받아서 반환
	      ...
	      try {
	            if(searchText != null && !searchText.equals("") ){
	            	sql +=" like '%"+searchText.trim()+"%' order by num desc where rownum <= 10";
	            }
	            PreparedStatement pstmt=conn.prepareStatement(sql);
