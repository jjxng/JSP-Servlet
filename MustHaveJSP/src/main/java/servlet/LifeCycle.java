...
public class LifeCycle extends HttpServlet
{
	/*
	 	...
	 	따라서 메서드명은 개발자가 결정한다. init()메서드가 호출되기 전 처리를
	 	위해 주로 사용한다.
	 */
	@PostConstruct
	...
	/* 서블릿 객체 생성 후 딱 한 번만 호출되는 메서드로, 보통 서블릿을 초기화 하는
	   역할을 한다.
	*/
	@Override
	...
	/*
	 	클라이언트의 요청을 분석하기 위해 호출된다. 전송방식에 상관없이 먼저
	 	호출된 후 doGet() 이나 doPost()를 호출한다.
	 */
	@Override
	...
	/*
	 	...
	 	이클립스에서는 server탭에서 서버를 종료하면 아래의 메서드가 호출된다.
	 */
	@Override
	public void destroy() {
	...
	/*
	 	destroy() 메서드 호출 후 후처리를 위해 사용된다. 어노테이션을 사용하므로
	 	메서드명은 우리가 결정하면 된다.
	*/
	@PreDestroy
