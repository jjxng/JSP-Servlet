...
{
	// 인수 생성자에서는 application내장객체를 매개변수로 전달한다.
	...
	public int selectCount(Map<String, Object> map)
	{
		...
		// 검색어가 있는 경우where절을 추가하여 조건에 맞는 게시물만 인출한다.
	...
	public List<BoardDTO> selectList(Map<String, Object> map)
	{
		...
		// 설정한다.
		// 게시판 목록은 출력 순서사 보장되어야 하므로 Set컬렉션은 사용할 수
		// 없고 List 컬렉션을 사용해야 한다.
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map)
	{
		...
		/*
		  	between 절 대신 비교연산자를 통해 쿼리문을 구성할 수 있다.
		  	=> where rNum>=? and rNum<=?
		 */
		try
		{
			...
			// 인파라미터를 설정한다. 구간의 기작과 끝을 계산한다.
			psmt.setString(1, map.get("start").toString());
			psmt.setString(2, map.get("end").toString());
			// 쿼리문을 실행하고 결과레코드를 ResultSet으로 반환받는다.
			rs = psmt.executeQuery();
			// 결과 레코드의 개수만큼 반복하여 List컬렉션에 저장한다.
			while (rs.next())
			{
				BoardDTO dto = new BoardDTO();
	...
	public BoardDTO selectView(String num)
	{
		...
		try
		{
			...
			rs = psmt.executeQuery();
			/*
				일련번호는 중복되지 않으므로 단 1개의 게시물만 인출하게 된다.
			...
			if (rs.next())
			{
				...
				/*
				 	각 컬럼의 값을 추출할 때 1부터 시작하는 인덱스와 컬럼명 둘 다
				 	사용할 수 있다. 날짜인 경우는 getData()메서드로 추출할 수 있다.
				 */
				dto.setContent(rs.getString("content"));
				dto.setPostdate(rs.getDate("postdate"));
				dto.setId(rs.getString("id"));
