...
/*
 	글쓰기 페이지로 진입시에는 단순한 페이지 이동이므로 deGet()이 요청을
 	처리한다. 즉, 글쓰기 페이지만 보여주면 된다.
 	내용을 모두 채운 후 작성(submit)할 때는 post방식으로 doPost()가 요청을
 	받아 처리한다. 
*/
public class WriteController extends HttpServlet
{

	//글쓰기 페이지로 진입할 때 요청을 처리한다.
	@Override
	...
	// 클라이언트가 작성한 내용을 DB에 입력하고 파일 업로드를 처리한다.
	@Override
			...
			throws ServletException, IOException
	{
		...
		/*
		 	파일 업로드를 위해 MultipartRequest객체를 생성한다. 객체가 정상적으로
		 	생성된다면 파일 업로드는 완료 된다.
		 */
		MultipartRequest mr = FileUtil.uploadFile(req, saveDirectory, maxPostSize);
		/*
		 	만약 객체생성이 되지 않았다면 업로드에 실패한 것이다. 이때는 파일의
		 	용량이나 디렉토리의 경로를 확인해 본다.
		 	만약 글 작성시 파일을 첨부하지 않았더라도 객체는 생성되므로 폼값은
		 	정상적으로 받을 수 있다.
		*/
		if (mr == null)
		{
			JSFunction.alertLocation(resp, "첨부 파일이 제한 용량을 초과합니다.",
					"../mvcboard/write.do");
			/*
			 	객체가 생성되지 않아 업로드에 실패하면 나머지 폼값도 받을 수
			 	없으므로 반드시 return을 통해 실행을 중지시켜야 한다.
			*/
			return;
		}
