...
public class PassController extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		/*
		 	...
		 	요청면은 이와 같으므로 파라미터를 받아오는 작업을 해야 한다.
		 	서블릿에서는 2가지 방법으로 받을 수 있는데
		 	방법1 : request내장객체를 이용한다. => req.getParameter(파라미터명)
		 	방법2 : View(JSP파일)에서 EL의 param으로 내장객체를 사용한다.
		 		=> ${param.파라미터명}
		*/
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
			throws ServletException, IOException
	{
		// 입력한 비밀번호와 hidden박스에서 저놋ㅇ된 파라미터 받기
		...
		if (confirmed)
		{
			if (mode.equals("edit"))
			{
				/*
				 	...
				 	게시물을 수정완료할 때까지 유지하고 수정이 완료되면 제거할
				...
				*/
				...
				/*
				 	...
				 	검증 없이 진입하였다면 세션에 저장된 패스워드가 없을 것이므로
