...
<body>
    ...
    <!-- 파일 업로드시 실패하는 경우 에러메세지를 표현한다. 해당 데이터는
    영역에 저장될 것이다. -->
    <span style="color: red;">${errorMessage }</span>
    
    <!-- 
    	파일 업로드를 위한 form태그 작성시 필수사항
    		1. method 즉, 전송방식은 반드시 post로 지정해야 된다.
    	...
    	파일 업로드를 위해 enctype을 위와같이 명시하는 경우
    		request내장객체로는 폼값을 받을 수 없다. cos.jar 확장 라이브러리
     ...
     <!-- 파일명은 그대로 하고, 중복되는 경우 자동으로 인덱스를 부여하여
     처리하는 방식 -->
<!--      <form name="fileForm" method="post" enctype="multipart/form-data" -->
