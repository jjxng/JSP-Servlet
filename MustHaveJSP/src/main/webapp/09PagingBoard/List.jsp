...
/*
	...
	해당 <form> 전송 방식은 get, action속성은 없는 상태이므로
	현재 페이지로 폼값이 전송된다.
*/
...
/*
	전체 페이지수를 계산한다.
	(전체 게시물 개수 / 페이지당 출력할 게시물 개수) => 결과값의 올림처리
	가령 게시물의 개수가 51개라면 나눴을 때 결과가 5.1이 된다. 이때 무조건
...
/*
	목록에 처음 진입했을 때 페이지 관련 파라미터가 없는 상태이므로 무조건
	1page로 지정한다. 만약 파라미터 pafeNum이 있다면 request내장객체를 통해
	받아온 후 페이지번호로 지정한다.
...
<!-- 공통 링크 -->
	...
	<!-- 앞에서 계산해둔 전체 페이지 수와 파라미터를 통해 얻어온 현재 페이지번호를
	출력한다. -->
	...
	<%
	    ...
	    {
	    	// 현재 출력할 게시물의 개수에 따라 출력번호는 달라지므로
	    	// totalCount를 사용하여 가상번호를 부여한다.
// 	    	virtualNum = totalCount--;
	    	
	    	// 현재 페이지 번호를 적용한 가상 번호 계산하기
	    	// 전체 게시물 수 - (((현재 페이지 - 1) * 한 페이지 출력 개수) + countNum증가치)
