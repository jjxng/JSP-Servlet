...
$(function() {
    ...
    $('#closeBtn').click(function() {
    	// 레이어 팝업창을 숨김처리 한다.
    	$('#popup').hide();
    	// 하루동안 열지 않음 체크박스에 체크한 경우에는 value를 얻어와서 변수에 저장
	    var chkVal = $("input:checkbox[id=inactiveToday]:checked").val();
    	// jAuery의 ajax()함수를 통해 요청값을 전송한다.
	    $.ajax({
	    	// 요청할 서버의 URL(경로)
	    	url : './PopupCookie.jsp',
	    	// 전송방식
	    	...
	    	// 즉, 체크를 했다면 1을 전송하고, 아니면 아무 값도 전송하지 않는다.
	    	data : {inactiveToday : chkVal},
	    	// 콜백 데이터의 타입은 text 형식으로 지정한다.
	    	dataType : "text",
	    	// 성공시 자동호출된 콜백 함수
	    	success : function(resData) {
	    		...
	    		// 콜백 데이터가 있다면 화면을 새로고침 한다. F5를 누른 것과 동일하다.
