...
<body>
	<!-- 
		쿠키 : 클라이언트의 상태 정보를 유지하기 위한 기술로 클라이언트의
			PC에 파일형태로 저장된다. 쿠키 하나의 크기는 4kb이고, 3000새까지
			만들 수 있다.
	 -->
	...
	/*
		...
		쿠키는 생성한 후 쿠키명을 변경하는 것은 불가능하다.
	*/
	Cookie cookie = new Cookie("myCookie", "쿠키맛나요");  // 쿠키생성
	/*
		쿠키의 경로 설정. 컨텐스트루트 경로로 지정하므로 웹 애플리케이션 전체에서
		사용할 수 있게 된다
	*/
	cookie.setPath(request.getContextPath());  // 경로를 컨텐스트 루트로 설정
	System.out.println("설정경로:" + request.getContextPath());
	cookie.setMaxAge(3600);  // 유지시간을 1시간으로 설정
	// 응답 헤더에 쿠키를 추가하여 클라이언트 쪽으로 전달한다.
	...
	*/
	...
	<%
	/*
		request내장객체의 getCockies()를 통해 현재 생성된 모든 쿠키를 배열의 형태로
		가져온다.
	*/
	Cookie[] cookies = request.getCookies();  // 요청 헤더의 모든 쿠키 얻기
	// 생성된 쿠키가 있다면 개수만큼 반복한다.
	if (cookies != null) {
		// 개선된 for문을 통해 쿠키를 읽어서
		for (Cookie c : cookies) {
			// 쿠키명과 쿠키값을 웹 브라우저에 출력한다.			
