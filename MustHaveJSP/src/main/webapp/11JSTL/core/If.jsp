...
<!-- 
 ...
 -->
	...
	<!-- 
		if(number % 2 == 0)과 같이 동일한 조건의 if문으로 해당 조건의 결과가
	...
	<!-- 
		...
		값에 대한 비교와 문자열에 대한 비교 모드를 할 수 있다.
		여기서 사용한 eq는 Java의 compareTo()와 같이 사전순으로 문자열을
		비교한다. 
	 -->
	<h4>문자열 비교과 else 구문 흉내내기</h4>
	<!-- 첫 번째 if문은 false의 결과가 나온다. -->
	<c:if test="${ string eq 'Java' }" var="result2">
		문자열은 Java입니다. <br />
	</c:if>
	<!-- result2에는 not을 붙여 반대의 조건을 만든다.(=>true)
	따라서 Java의 else문과 같은 구문이 된다. -->
	<c:if test="${ not result2 }"><!-- => true 값이므로 아래 문장이 출력됨 -->
		'Java'가 아닙니다. <br />
	</c:if>
	
	<h4>조건식 주의사항</h4>
	<c:if test="100" var="result3">
		EL이 아닌 정수를 지정하면 false<!-- false이므로 출력이 안 된다. -->
	</c:if>
	result3 : ${result3 } <br />
	<c:if test="tRuE" var="result4">
		대소문자 구분 업시 "tRuE"인 경우 true <br />
	</c:if>
	result4 : ${ result4 } <br />
	
	<!--  test에는 앞뒤에 하나라도 공백이 들어가면 무조건 false를 반환받게 된다.
	따라서 주의해야 한다. -->
	<c:if test=" ${true} " var="result5">
		EL 양쪽에 빈 공백이 있는 경우 false <br /><!-- test="{ true }"이면 true. 출력된다. -->
	</c:if>
	result5 : ${ result5 } <br />
	...
	<!-- 
		...
		받은 후 seoulit/1234인 경우에는 'seoulit님, 반갑습니다.~'이라고 출력한다.
	 ...
	 <!-- 최초 실행 시에는 파라미터가 없는 상태이므로 아무 내용도 출력되지 않는다. -->
	<c:if test="${ not empty param.user }">
		전송된 아이디 : ${param.user}<br/>
		전송된 패스워드 : ${param.pass}<br/>
		<!-- 
			아이디, 패스워드를 입력했을 때만 일치여부를 판단한 후 결과를 변수에
			저장한다.
			if태그는 else구문이 따로 없으므로 not을 통해 두 번째 if문을 구성해야
			한다.
		 -->
	</c:if>
