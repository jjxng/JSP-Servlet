...
<body>
	...
	lists.add(new Person("장열실", 44));
	lists.add(new Person("신숙주", 54));
	%>
	<!-- set태그로 page영역에 속성(변수)을 저장한다. -->
	<c:set var="lists" value="<%= lists %>"/>
	<!-- 해당 변수를 통해 확장 for문을 실행한다. items에 저장한 컬렉션에 저장된
	객체의 개수만큼 반복하여 list로 하나씩 전달한다. -->
	...
	for(String key : keys){
		...
		// Key와 Value를 출력한다. 여기서 Value는 Person객체이므로 멤버변수를 통해
		// 값을 출력할 수 있다.
		out.println("Key=>"+ key +"<br>");
		out.println("Value=>"+ p.getName() +","+ p.getAge() +"<br>");
	}
	/*
		JSTL의 확장 for문을 사용하면 Key를 별도로 얻어와야 하는 번거로움이 없으므로
		편리하다.
	*/
	%>
</body>
