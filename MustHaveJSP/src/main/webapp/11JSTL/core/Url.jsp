...
<body>
<!-- 
	url 태그 : url을 생성할 때 사용한다. 절대경로로 사용시 컨텍스트루트 경로는
		제외한다. var속성을 지정하지 않는 경우 해당 위치에 생성된 url이 출력된다.
		para태그로 지정한 값은 쿼리스트링으로 URL뒤에 연결된다.
 -->
	<h4>url 태그로 링크 걸기</h4>
	<!-- HOST(도메인 부분)를 제외한 나머지 경로를 반환한다. var속성이
